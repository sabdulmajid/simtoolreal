...
log_path="logs/launch_train_gpu0_${stamp}.log"
metrics_path="reports/metrics/launch_train_gpu0_${stamp}.json"
report_path="reports/launch_train_gpu0_${stamp}.json"
...
for module in tyro isaacgym rl_games.torch_runner; do
  if ! require_python_module "${module}"; then
    ...
    write_json_report "${report_path}" "dependency_error" 1 "${message}" "bash scripts/launch_train_gpu0.sh" "${log_path}" "${started_at}" "$(simtoolreal_timestamp)" "${extra_json}"
    exit 1
  fi
done
...
EXPERIMENT_NAME="${EXPERIMENT_NAME:-1_gpu0_train}"
...
if [[ "${exit_code}" -eq 0 ]]; then
  status="success"
  message="GPU0 training command completed."
  ...
  message="GPU0 training reached CUDA execution, but torch cannot run kernels on this GPU. See log."
elif grep -Eqi "ModuleNotFoundError|No module named" "${log_path}"; then
  status="dependency_error"
  message="GPU0 training failed on a missing Python dependency. See log."
else
  status="failed"
  message="GPU0 training command failed. See log."
fi
