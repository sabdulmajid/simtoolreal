...
if [[ "${exit_code}" -eq 0 ]]; then
  ...
  message="Finetune smoke reached CUDA execution, but torch cannot run kernels on the visible GPU. See log."
elif grep -Eqi "ModuleNotFoundError|No module named" "${log_path}"; then
  status="dependency_error"
  message="Finetune smoke failed on a missing Python dependency. See log."
else
  status="failed"
